...
# python part_b_multiclass.py --dataset_root 'dataset_for_A2//dataset_for_A2//multi_dataset' --save_weights_path "weights_b"
...
def backward_pass(X, Y, A1, A2, A3, A4, W1, W2, W3, W4):
    ...
    dZ4 = (A4 - Y ) # Shape (n, 8)
    dW4 = np.dot(A3.T, dZ4)/m # Shape (128, 8)
...
def init_params():
  ...
  W4 = np.random.randn(128, 8) * np.sqrt(2 / (128))
  b4 = np.zeros((1, 8)) # row vector
...
if __name__ == '__main__':
    ...
    Y = pd.DataFrame(label_list, columns=['Label']).to_numpy().ravel() 
    Y = one_hot(Y)
